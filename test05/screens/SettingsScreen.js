...
export default function SettingsScreen() {
  const renderRow = ({ item }) => {
    return <ListItem onPress={log} title={item.title} leftIcon={{ name: item.icon }} chevron bottomDivider />
  }
  ...
  return (
    <ScrollView>
      ...
      <View style={styles.list}>
        {list2.map((l, i) => (
          <ListItem leftAvatar={{ title: l.name[0], source: { uri: l.avatar_url } }} key={i} onPress={log} title={l.name} subtitle={l.subtitle} chevron bottomDivider />
        ))}
      </View>
      <View style={styles.list}>
        {list2.map((l, i) => (
      ...
      <View style={styles.list}>
        <ListItem title="Name" input={{ placeholder: "Type your name" }} chevron bottomDivider />
        ...
        <ListItem
          ...
          bottomDivider
        />
        <ListItem title="Check that please 😢" checkBox={{ checked: true }} bottomDivider />
        <ListItem title="With a Badge ! 😻" badge={{ value: "12" }} bottomDivider />
      ...
      <View style={styles.list}>
        <ListItem
          ...
          leftAvatar={{ source: require("../images/water.png") }}
        />
      </View>

      <View style={styles.list}>
        <FlatList data={list1} keyExtractor={(a) => a.title} renderItem={renderRow} />
      </View>

      <View style={[styles.headerContainer, { backgroundColor: "#FF9800", marginTop: 20 }]}>
      ...
      <View style={styles.container}>
        ...
        >
          {_.chunk(list3, 3).map((chunk, chunkIndex) => (
            ...
            >
              {chunk.map((l, i) => (
                <Avatar large rounded source={l.image_url ? { uri: l.image_url } : null} icon={l.icon} title={l.title} key={`${chunkIndex}-${i}`} />
              ))}
            </View>
          ))}
        </Card>
